...
**Android-specific watch-fors:**
- Now in Android is **multi-module**. Does the candidate map the module graph (`:feature:bookmarks`, `:core:data`, `:core:domain`, etc.) or do they get lost across module boundaries?
- Do they identify the **state-management pattern** (ViewModel + `StateFlow` + `collectAsStateWithLifecycle`) before changing UI?
- Do they spot the **repository layer** (`UserNewsResourceRepository`) before re-implementing data access?
...
**Android-specific watch-fors:**
- Do they specify **Compose vs. Views**, **Material 3 vs. M2**, and target `minSdk` / `compileSdk` in the prompt?
- Do they pull in the relevant **module's `build.gradle.kts`** so the AI knows available dependencies?
- Do they reference existing **Composables and theme tokens** (e.g., `NiaTheme`, `NiaIconButton`) to keep the AI within the design system?
...
**Android-specific watch-fors:**
- Do they split work by **layer** (UI → ViewModel → repository) rather than dropping everything in the Composable?
- Do they consider **`@Preview` composables** or screenshot tests as a verification step?
- Do they decide upfront which **module** the change belongs in?
...
**Android-specific watch-fors:**
- Catches **hallucinated Compose APIs** (made-up modifiers, non-existent Material 3 components).
- Spots wrong **Hilt scopes** (`@Singleton` where a `@ViewModelScoped` is needed, missing `@HiltViewModel`).
- Catches **deprecated APIs** the AI loves to suggest: `LiveData` where the project uses `StateFlow`, `lifecycleScope.launchWhenStarted`, `viewModel()` instead of `hiltViewModel()`.
- Notices **missing `collectAsStateWithLifecycle`** (vs. `collectAsState`) for lifecycle-aware collection.
- Spots **threading mistakes** — calling suspend functions from the wrong context.
...
**Android-specific watch-fors:**
- When the **Gradle build fails** (KSP, version catalog, Hilt codegen), do they read the actual error or just retry the same prompt?
- When Compose previews break, do they distinguish **preview-only failures** from runtime failures?
- Do they know when to **invalidate caches / restart** vs. when it's a code problem?
